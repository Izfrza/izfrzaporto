...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (track1 && track2) {
        // Menggandakan konten di dalam track agar loop berjalan tanpa patah/putus
        track1.innerHTML += track1.innerHTML;
        track2.innerHTML += track2.innerHTML;

        let xPos1 = 0;
        let xPos2 = -50; // Dimulai dari posisi minus agar pergerakan baris kedua bervariasi

        let lastScrollY = window.scrollY;
        let scrollVelocity = 0;
        let targetVelocity = 0;

        // Base speed (Kecepatan konstan saat halaman diam tidak di-scroll)
        const baseSpeed = 0.08;

        function updateAnimation() {
            // Menggunakan algoritma LERP (Linear Interpolation) untuk efek perlambatan yang smooth
            scrollVelocity += (targetVelocity - scrollVelocity) * 0.05;

            // Baris 1 bergerak ke kiri (ditambah efek scroll velocity)
            xPos1 -= (baseSpeed + scrollVelocity);
            // Baris 2 bergerak ke kanan (dikurang efek scroll velocity agar arahnya berlawanan)
            xPos2 += (baseSpeed + scrollVelocity);

            // Reset koordinat ke 0 untuk loop infinite jika sudah lewat 50%
            if (xPos1 <= -50) xPos1 = 0;
            if (xPos2 >= 0) xPos2 = -50;

            track1.style.transform = `translateX(${xPos1}%)`;
            track2.style.transform = `translateX(${xPos2}%)`;

            // Kurangi target secara bertahap agar kembali normal saat scroll berhenti
            targetVelocity *= 0.95;
        ...
        window.addEventListener('scroll', () => {
            ...
            targetVelocity = Math.min(delta * 0.15, 4.0);
...
const planets = document.querySelectorAll('.planet, .sun-linux');
const hudDisplay = document.getElementById('current-target');

planets.forEach(planet => {
    const name = planet.getAttribute('title');

    planet.addEventListener('mouseover', () => {
        hudDisplay.innerText = name;
    });
